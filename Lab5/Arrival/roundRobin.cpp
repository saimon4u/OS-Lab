...
void makeReadyQueue(vector<Process> &ready, int currTime, vector<Process> processes){
    ...
    sort(ready.begin(), ready.end(), compareTwoProcess);
}
...
int main(){
    ...
    if(currTime < temp[0].arrivalTime){
        Result result;
        result.name = "Dl";
    ...
    for(auto t: temp){
        if(t.arrivalTime <= currTime){
            q.push(t);
        }
    }

    while(!temp.empty()){
        Process currP = q.front();
        q.pop();
        // cout << currP.name << endl;
        int range = 0;
        if(currP.burstTime <= quantum){
                Result result;
        ...
        for(auto t: temp){
            if(t.arrivalTime > currTime-range and t.arrivalTime <= currTime){
                q.push(t);
            }
        }
